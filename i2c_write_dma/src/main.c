...
static SemaphoreHandle_t gh_lock = NULL;
...
static void _i2c_write(uint8_t addr, uint8_t* data, uint8_t dataLen)
{
    ...
    {
        LOG_E(common, "Semaphore lock failed.\n");
    }
}
...
static void _i2c_callback(uint8_t slave_address, hal_i2c_callback_event_t event, void *user_data)
{
    ...
    if (event == HAL_I2C_EVENT_SUCCESS)
    {
        ...
        {
            LOG_E(common, "Release semaphore failed.\n");
        }
    }
    // error
    else
    {
        LOG_E(common, "_i2c_callback() error.\n");
    }
}
...
static void lcd_init(void)
{
    ...
    ret = hal_i2c_master_register_callback(HAL_I2C_MASTER_0, _i2c_callback, NULL);
    if (ret != HAL_I2C_STATUS_OK)
    {
        LOG_E(common, "Register I2C callback failed.\n");
        exit(1);
    }
...
int main(void)
{
    ...
    if (gh_lock == NULL)
    {
        LOG_E(common, "Semaphore creation failed.\n");
        exit(1);
    }
    ...
    {
        LOG_E(common, "Semaphore release failed.\n");
        exit(1);
    }
    // lock the semaphore to block the subsequent take
    if (xSemaphoreTake(gh_lock, portMAX_DELAY) != pdTRUE)
    {
        LOG_E(common, "Semaphore take failed.\n");
        exit(1);
    }
